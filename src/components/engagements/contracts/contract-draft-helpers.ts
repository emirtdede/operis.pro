...
export function getDpaButtonLabel(enabled: boolean, isTr: boolean): string {
  if (enabled) return isTr ? "KVKK EK-2 (Aktif)" : "KVKK ANNEX-2 (Active)";
  return isTr ? "6698 KVKK & DPA Ekle" : "Add KVKK DPA";
}

export function getSafeHarborButtonLabel(enabled: boolean, isTr: boolean): string {
  if (enabled) return isTr ? "İş K. 8 EK-3 (Aktif)" : "Labor Safe Harbor (Active)";
  return isTr ? "İş K. 8 Kalkanı Ekle" : "Add Labor Shield";
}
...
export function getWhiteLabelButtonLabel(enabled: boolean, isTr: boolean): string {
  if (enabled) return isTr ? "Kurumsal Mod (Açık)" : "White-Label (Active)";
  return isTr ? "Kurumsal Sade Mod" : "White-Label Mode";
}
